...
## Exercise: 8. Authors view

Through these exercises, we'll implement a frontend for the GraphQL library. The application base can be found in the library-frontend folder. Remember to install the dependencies with the command `npm install`.
...
## Exercise: 11. Authors birth year
...
## Exercise: 12. Authors birth year advanced

Make the birth year form such that the birth year can be set via a dropdown only for an existing author. Use the [select element](https://react.dev/reference/react-dom/components/select).
