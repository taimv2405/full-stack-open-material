# Full Stack Open: GraphQL — offline export

This is an AI-friendly export of the publicly available course material. Markdown preserves reading order; `assets/` contains the original downloaded images; `raw-pages.json` and `raw-exercises.json` preserve the API blocks for fidelity.

## Chapters

- [Chapter 1: Getting Started](./chapter-01-getting-started.md)
- [Chapter 2: GraphQL server](./chapter-02-graphql-server.md)
- [Chapter 3: React and GraphQL](./chapter-03-react-and-graphql.md)
- [Chapter 4: Database and user administration](./chapter-04-database-and-user-administration.md)
- [Chapter 5: Login and updating the cache](./chapter-05-login-and-updating-the-cache.md)
- [Chapter 6: Fragments and subscriptions](./chapter-06-fragments-and-subscriptions.md)

Source: https://courses.mooc.fi/org/uh-cs/courses/full-stack-open-graphql
Exported: 2026-08-27T10:29:32.646Z
